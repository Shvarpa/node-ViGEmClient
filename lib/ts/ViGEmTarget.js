...
class ViGEmTarget extends events_1.EventEmitter {
    ...
    connect(opts = {}) {
        ...
        let error = Utils_1.handlePossibleError(Client_1.vigemclient.vigem_target_add(client, target));
        if (!error) {
    ...
    disconnect() {
        this.checkConnection();
        let client = this.client.handle;
        let error = Utils_1.handlePossibleError(Client_1.vigemclient.vigem_target_remove(client, this.target));
        if (!error) {
            this._connected = false;
